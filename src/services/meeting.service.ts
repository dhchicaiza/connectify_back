 ...
 * @param meetingId - Meeting ID
 * @param summary - Summary data generated by AI
 */
export async function saveMeetingSummary(meetingId: string, summary: any): Promise<void> {
  try {
    const db = getFirestore();
    const meetingRef = db.collection(COLLECTIONS.MEETINGS).doc(meetingId);
    const meetingDoc = await meetingRef.get();
    ...
    // Save summary to a subcollection or as a field in the meeting document
    await meetingRef.update({
      summary: summary,
    ...
    logger.error('Error saving meeting summary', error);
    throw error;
  }
}
